...
<table  style="padding:5em; background-color:#e7f1f6; border:0px solid #FF0000"  cellpadding="1" cellspacing="3"   width='100%'>
	...
	<tr>
		<td bgcolor='white' colspan='4' align=center><input type=button value='Nova newsletters' onclick='formEmail();'></td>
	</tr>


	<tr>
		<td bgcolor='white' colspan='4' align=center><font class='tituloAdv'>Total de inscritos <b> <?=$datcount["count"]?><b>&nbsp;&nbsp;Newsletters NegociosLucrativos.com</font></td>
	</tr>
...
<?
	...
	for ($i=0;$i<$_rows_;$i++)   {
		 ...
		 else if( $line[status] == 'E' )
		{
		 ...
		 echo "<td $bgcolor> $ahref ".$line["subject"]."</td>";
		 echo "<td $bgcolor> $ahref ".format_date($line["dtSend"])."</td>";
		 echo "<td $bgcolor> $ahref ".$linkStatus."</td>";
		 echo "<td $bgcolor>$ahref ". $linkModify."</td>";
		...
		<div id="rodapePaginacao" class="rodapePaginacao">
			<ul> 
				...
				for ($i=0; $i < $qtdPaginas; $i++){
					...
					?>
					<li class="<?=$class?>">
						<a href="mvcAnnouncement.php?action=listBusinessBuy&<?=$post."&paginaAtual=".$i."&totalPaginas=".$qtdPaginas."&totalRegistros=".$totalRegistros?>" class="<?=$class?>">
						<?=$i+1?> 
						</a>
					</li> 
					<?php
