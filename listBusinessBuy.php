...
if( $_REQUEST['lupa_x'] || $_REQUEST["txtSearch"] ){
	...
	while( list( $key, $value ) = each( $Awork_search ) ) {
		 if( $value  && strlen($value) > 2  )              
			 $qryWhere .=   " and a.title like '%$value%' and a.description like '%$value%'";
	}
	if($qryWhere)	
	{
...
$qry = "select count(*)  from announcement a where typeannouncement='B'  $qryWhere ";
...
<table  style="padding:5em; background-color:#e7f1f6; border:0px solid #FF0000"  cellpadding="1" cellspacing="3"   width='100%'>
	...
	<tr>
		<td bgcolor='white' colspan='4' align=center><font class='tituloAdv'>Compradores e Investidores em empresas </font></td>
	</tr>
	<tr>
...
<?
	...
	$qry = "select a.id,a.title,a.sector,a.typecompany,a.priceselling,datainc,a.price  from announcement a where typeannouncement='B' $qryWhere   order by datainc desc $limit ";
...
<tr>
	<td colspan='4' bgcolor='white' align='center'> <font class='subtitulo3'>Total de <?=$conta_linhas?> anúncios de compradores e investidores em empresa</font>
